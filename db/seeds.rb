...
].each do |event_name, file_image|
  puts "Start Setting Images For Event: #{event_name}, File: #{file_image}"
  e = Event.find_by(name: event_name)
  f = File.open(Rails.root.join("app/assets/images/#{file_image}"))
  e.main_image.attach(io: f, filename: file_image)
  sleep 1.second
  puts "Done Setting Images For Event: #{event_name}, File: #{file_image}"
end

User.create!({name: "Mark", email: 'mark@example.com', password: 'secret', password_confirmation: 'secret', admin: true} )
